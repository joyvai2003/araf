...
const SmartAssistant: React.FC<Props> = ({ liveEntries, expenses, language }) => {
  const [advice, setAdvice] = useState<string>('');
  const [loading, setLoading] = useState(false);

  const handleGetAdvice = async () => {
    if (liveEntries.length === 0 && expenses.length === 0) {
      setAdvice(language === 'bn' ? "পরামর্শ দেওয়ার জন্য আপনার কিছু হিসাব থাকা প্রয়োজন। আগে কিছু এন্ট্রি করুন!" : "Need some data to analyze! Add entries first.");
      return;
    }
    setLoading(true);
    // Note: service always takes the logic, advice can be translated by AI if specified in prompt
    ...
    <div className="space-y-6 animate-in slide-in-from-bottom duration-500">
      <div className="bg-gradient-to-br from-slate-800 to-slate-900 p-8 rounded-[2rem] text-white shadow-2xl relative overflow-hidden">
        <div className="relative z-10">
          <div className="bg-emerald-500 w-12 h-12 rounded-2xl flex items-center justify-center mb-4 text-2xl">🤖</div>
          <h2 className="text-2xl font-black mb-2 tracking-tight">{language === 'bn' ? 'স্মার্ট এআই সহকারী' : 'Smart AI Assistant'}</h2>
          <p className="opacity-70 mb-8 max-w-sm text-sm leading-relaxed">
            {language === 'bn' ? 'আপনার দোকানের আয়-ব্যয় বিশ্লেষণ করে জেমিনি এআই আপনাকে বিজনেস গ্রোথ টিপস দেবে।' : 'Gemini AI will analyze your shop transactions and provide growth tips.'}
          </p>
          <button 
          ...
          >
            {loading ? (language === 'bn' ? 'অ্যানালাইজ করা হচ্ছে...' : 'Analyzing...') : (language === 'bn' ? 'পরামর্শ নিন' : 'Get Advice')}
          </button>
        </div>
        ...
        <div className="bg-white p-8 rounded-[2rem] shadow-sm border border-slate-100 animate-in fade-in slide-in-from-bottom-4 duration-500">
          <div className="flex items-center gap-3 mb-6 border-b pb-4">
            <h3 className="font-black text-slate-800 uppercase tracking-wider text-xs">
              {language === 'bn' ? 'জেমিনির বিজনেস রিপোর্ট' : "Gemini's Business Report"}
            </h3>
          </div>
